...
const ErrorsField = ({children, ...props}, {uniforms: {error, schema}}) => {
        return (!error && !children) ? nothing : (
            <div {...filterDOMProps(props)}>
                ...
                {children}
                <div dataShow="true" className="ant-alert ant-alert-error ant-alert-with-description"><i
                    className="anticon anticon-close-circle ant-alert-icon">
                    <svg viewBox="64 64 896 896" className="" data-icon="close-circle" width="1em" height="1em"
                    ...
                    <ul>
                        {schema.getErrorMessages(error).map((message, index) =>
                            <li key={index} className="ant-alert-description">
                                {message}
                            </li>
                        )}
                    </ul>
