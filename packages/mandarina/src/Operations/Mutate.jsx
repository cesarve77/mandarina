...
var Mutate = /** @class */ (function (_super) {
    ...
    function Mutate() {
        ...
        _this.buildQueryFromFields = function () { return utils_1.buildQueryFromFields(_this.props.table.getFields()); };
        _this.refetchQueries = function (mutationResult) {
            var refetchQueries = [];
            var _a = _this.props.table.names.query, single = _a.single, plural = _a.plural, connection = _a.connection;
            // @ts-ignore
     ...
     */
    Mutate.prototype.spider = function (obj, table, wrapper, initiator) {
        var _this = this;
        if (typeof obj !== "object" || obj === undefined || obj === null)
            return obj;
        if (Array.isArray(obj)) {
            return obj.map(function (obj) { return (_this.spider(obj, table, wrapper, initiator)); });
        }
        else {
            var data_1 = initiator(obj, table);
            Object.keys(obj).forEach(function (key) {
                var value = obj[key];
                var definition = table.schema.getFieldDefinition(key);
                if (typeof value === "object" && value !== null && value !== undefined && !(value instanceof Date)) {
                    if (Array.isArray(definition.type)) {
                        if (typeof definition.type[0] === 'string') {
                            var table_1 = Table_1.Table.getInstance(definition.type[0]);
                            data_1[key] = wrapper(_this.spider(value, table_1, wrapper, initiator), table_1);
                        }
                        else {
                            var native = definition.type[0];
                            data_1[key] = wrapper(_this.spider(value, table, wrapper, initiator), native);
                        }
                    }
                    else {
                        if (typeof definition.type === 'string') {
                            var table_2 = Table_1.Table.getInstance(definition.type);
                            data_1[key] = wrapper(_this.spider(value, table_2, wrapper, initiator), table_2);
                        }
                        else {
                            data_1[key] = wrapper(_this.spider(value, table, wrapper, initiator), definition.type);
                        }
                    }
                }
                else {
    ...
    Mutate.prototype.getTypesDoc = function (obj, table) {
        var _this = this;
        var wrapper = function (result) { return result; };
        var initiator = function (obj, table) { return ({
    ...
    Mutate.prototype.mutate = function (model, mutationFn) {
        var _a;
        var _b = this.props, table = _b.table, where = _b.where, type = _b.type, optimisticResponse = _b.optimisticResponse;
...
exports.Create = function (_a) {
    var table = _a.table, optimisticResponse = _a.optimisticResponse, props = __rest(_a, ["table", "optimisticResponse"]);
    return (<MutateWithApollo type='create' table={table} optimisticResponse={optimisticResponse} {...props}/>);
};
exports.Update = function (_a) {
    var id = _a.id, table = _a.table, children = _a.children, fields = _a.fields, optimisticResponse = _a.optimisticResponse, props = __rest(_a, ["id", "table", "children", "fields", "optimisticResponse"]);
    return (<Find_1.FindOne table={table} where={{ id: id }} fields={fields} {...props}>
        {function (_a) {
        var data = _a.data, findOneProps = __rest(_a, ["data"]);
        return (<MutateWithApollo where={{ id: id }} type='update' table={table} doc={data} optimisticResponse={optimisticResponse} {...findOneProps}>
                {children}
            </MutateWithApollo>);
    }}
    </Find_1.FindOne>);
